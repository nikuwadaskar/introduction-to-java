...
class EdgeCN implements Comparable<EdgeCN>{
	int source;
	int destination;
	int weight;
	public int compareTo(EdgeCN o){
...
public class KruskalsAlgorithm {
	...
	public static void kruskals(EdgeCN input[],int V){
		Arrays.sort(input);
		int count=0;
		int k=0;
		EdgeCN output[]=new EdgeCN[V-1];
		int parent[]=new int[V];
		for(int j=0;j<V;j++){
			parent[j]=j;
		}
		while(count!=V-1){
			EdgeCN currentEdge=input[k];
